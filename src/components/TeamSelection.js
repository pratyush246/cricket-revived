...
const TeamSelection = ({  players, yesVoters, captains, username }) => {
  ...
  // --- Render ---
    return (
    <div className="w-full max-w-6xl mx-auto p-6 md:p-10 bg-gradient-to-br from-green-50 via-yellow-50 to-blue-50 rounded-3xl shadow-2xl border-4 border-green-200 flex flex-col gap-8 items-center animate-fade-in">
      <h1 className="text-4xl font-extrabold text-green-700 mb-4 text-center drop-shadow-lg tracking-tight">Team Selection</h1>
      <div className="flex flex-col md:flex-row gap-8 w-full justify-center mb-8">
        {captains?.map((cap, idx) => (
          <div
            key={cap}
            className={`flex-1 min-w-[260px] max-w-xs mx-auto bg-gradient-to-br from-white via-green-50 to-blue-50 rounded-3xl shadow-2xl border-4 ${idx === pickingCaptainIdx ? 'border-yellow-400 ring-4 ring-yellow-300 animate-glow' : 'border-green-200'} p-6 flex flex-col items-center transition-transform duration-300 ${idx === pickingCaptainIdx ? 'scale-105' : 'hover:scale-105'}`}
            style={{ boxShadow: idx === pickingCaptainIdx ? '0 0 32px 8px #fde68a, 0 2px 16px #4ade80' : '0 2px 16px #4ade80' }}
          >
            <div className={`text-3xl font-extrabold mb-2 ${idx === pickingCaptainIdx ? 'text-yellow-700' : 'text-green-700'} drop-shadow-lg`}>{cap}{idx === pickingCaptainIdx && <span className="ml-2 text-lg font-bold text-yellow-600 animate-pulse">(Picking)</span>}</div>
      ...
      {/* MVP points summary */}
      <div className="flex flex-col md:flex-row gap-6 justify-center mt-8 w-full">
        {captains?.map((cap, idx) => (
          <div
            key={cap}
